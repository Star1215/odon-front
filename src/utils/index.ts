...
import { moonriver } from '@pancakeswap/wagmi'
import memoize from 'lodash/memoize'
import { TokenAddressMap } from 'state/types'
import { BASE_BSC_SCAN_URLS, BASE_MOON_SCAN_URLS } from '../config'
...
export function getBlockExploreLink(
  ...
  const chainId = chainIdOverride || ChainId.MOONRIVER
  const chain = chains.find((c) => c.id === chainId)
  if (!chain) return moonriver.blockExplorers.default.url
  switch (type) {
    case 'transaction': {
...
export function getBlockExploreName(chainIdOverride?: number) {
  ...
  return chain?.blockExplorers?.default.name || 'MoonScan'
}

export function getBscScanLinkForNft(collectionAddress: string, tokenId: string): string {
  return `${BASE_MOON_SCAN_URLS[ChainId.MOONRIVER]}/token/${collectionAddress}?a=${tokenId}`
}
