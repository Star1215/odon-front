  ...
  return (
    <Modal title={title || t('Check your network')} hideCloseButton headerBackground="gradients.cardHeader">
      <Grid style={{ gap: '16px' }} maxWidth="360px">
        <Text bold>{t('It’s a Moonriver Network only feature')}</Text>
        ...
        <Text small>
          {t(
            'Our Pools, Limit, Trading Competition, Prediction, Lottery and NFTs features are currently available only on Moonriver Chain! Come over and join the community in the fun!',
          )}
        </Text>
        {canSwitch && (
          <Button
            ...
            isLoading={isLoading}
            onClick={() => switchNetworkAsync(ChainId.MOONRIVER)}
          >
            {t('Switch to %chain%', { chain: 'Moonriver Network' })}
          </Button>
        )}
        {isConnected && (
          <Button
