...
const nonBnbPools = poolsConfig.filter((pool) => pool.stakingToken.symbol !== 'MOVR')
const bnbPools = poolsConfig.filter((pool) => pool.stakingToken.symbol === 'MOVR')
...
export const fetchUserBalances = async (account) => {
  ...
  const [tokenBalancesRaw, bnbBalance] = await Promise.all([
    ...
    moonRpcProvider.getBalance(account),
  ])
