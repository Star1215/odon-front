...
export const fetchCakePoolUserDataAsync = (account: string) => async (dispatch) => {
  const allowanceCall = {
    address: moonTokens.cake.address,
    name: 'allowance',
  ...
  try {
    ...
    const bnbBusdFarm =
      activePriceHelperLpsConfig.length > 0
        ? farmsData.find((farm) => farm.token.symbol === 'BUSD' && farm.quoteToken.symbol === 'WMOVR')
        : null
    ...
    const liveData = poolsConfig.map((pool) => {
      ...
      const earningTokenPrice = earningTokenAddress ? prices[earningTokenAddress] : 0
      // const poolTVL = stakingTokenPrice.times(getBalanceNumber(new BigNumber(totalStaking.totalStaked), pool.stakingToken.decimals))
      // poolsTVL = poolsTVL.plus(poolTVL)
...
>('pool/fetchPoolsUserData', async (account, { rejectWithValue }) => {
  ...
  try {
    console.log('debug pools user st1')
    ...
    console.log('debug pools userData', userData)
    return userData
  } catch (e) {
