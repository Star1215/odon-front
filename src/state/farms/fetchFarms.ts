...
function farmLpTransformer(farmResult, masterChefResult) {
  return (farm, index) => {
    ...
    const depositFee = info.depositFeeBP ?? BIG_ZERO
    const withdrawFee = info.withdrawFeeBP ?? BIG_ZERO
    const lockTime = info.lockTime ? new BigNumber(info.lockTime?._hex) : BIG_ZERO
