...
import { moonRpcProvider } from 'utils/providers'
...
const useTokenBalance = (tokenAddress: string, forceBSC?: boolean) => {
  ...
  const key = useMemo(
    () =>
      account
        ? {
            contract: forceBSC ? contract.connect(moonRpcProvider) : contract,
...
export const useGetBnbBalance = () => {
  ...
  const { status, data, mutate } = useSWR([account, 'bnbBalance'], async () => {
    return moonRpcProvider.getBalance(account)
  })
...
export const useGetCakeBalance = () => {
  ...
  const { balance, fetchStatus } = useTokenBalance(CAKE[chainId]?.address || CAKE[ChainId.MOONRIVER]?.address, true)
